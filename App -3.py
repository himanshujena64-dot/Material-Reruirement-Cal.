    ...
    if bom_file and req_file:
        if st.sidebar.button("🚀 Run MRP Engine"):
            ...
            for lvl in range(1, max_level + 1):
                ...
                def calc_req(row):
                    if row["Is_Phantom"]:
                        return row["Demand"]  # Pass-through (no multiplication)
                    else:
                        return row["Demand"] * row["Required Qty"]

                merged["Gross_Req"] = merged.apply(calc_req, axis=1)

                # 🔥 REMOVE PHANTOM FROM RESULT BUT KEEP FLOW
                ...
                # 🔥 Only store NON-phantom in final result
                non_phantom = bom[["Component", "Is_Phantom"]].drop_duplicates()
                grouped = grouped.merge(non_phantom, on="Component", how="left")

                results.append(grouped[~grouped["Is_Phantom"]])

                # 🔥 Continue explosion with ALL (including phantom pass-through)
            ...
            if results:
                ...
                pivot = all_data.groupby(
                    ["Component", "Month"]
                )["Required"].sum().unstack().fillna(0).reset_index()
