...
Includes component search bar with demand summary + ancestry flowchart.
"""
...
with st.sidebar:
    ...
    prod_file = st.file_uploader("Production Orders (.xlsx) — optional",
                                 type=["xlsx","xls"], key="prod")
    run_btn   = st.button("▶ Run MRP", type="primary", use_container_width=True)


# ═══════════════════════════════════════════════════════════════
# SHARED HELPERS  (identical logic to Colab version)
# ═══════════════════════════════════════════════════════════════
MONTH_ABBR = {
...
def get_ancestry_paths(component, bom):
    """
    Walk up the BOM tree to find every FG → ... → component path.
    Returns list of dicts with fg, alt, level, path_comps, path_descs,
    path_qtys, path_sp (special procurement per node).
    """
...
def build_dot_tree(component, paths, req_df, result_dfs, months, stock, prod_summary):
    """
    Build a Graphviz DOT string for the ancestry tree of a component.

    Node colours:
      - FG (BOM Header)          : steel blue
      - Intermediate normal node : gold
      - Phantom node             : coral / dashed border
      - Target component         : lime green
    """
    # Aggregate total demand per (fg, alt) across all months
    fg_demand = {}
    for path in paths:
    ...
    # Aggregate total gross / shortage for each component across result_dfs
    gross_map    = {}
    shortage_map = {}
    ...
    node_attrs = {}   # node_id → DOT attribute string
    ...
    for path in paths:
        ...
        for i, (comp, desc, qty, sp) in enumerate(
            zip(path["path_comps"], path["path_descs"],
...
def show_search_section(bom, req_df, months, stock, prod_summary,
    ...
    # Find which levels this component appears in
    found_in = {}
    ...
    # ── Monthly demand / shortage table ──────────────────────────
    if found_in:
        st.markdown("#### Monthly demand & shortage")

        # Combine rows from all levels (a component can only be at one level, but safe)
        ...
        s4.metric("Months with shortage",
                  f"{(monthly['Shortage']>0).sum()} / {len(monthly)}")
    else:
        st.info("Component found in BOM but not in MRP results "
                "(no demand in requirement file or phantom).")

    # ── Ancestry tree ─────────────────────────────────────────────
    st.markdown("#### BOM ancestry tree")
    st.caption("FG → intermediate levels → searched component (green). "
               "Phantom nodes shown with dashed border.")
    ...
    # Graphviz tree
    # Limit to first 12 paths to keep tree readable
    MAX_PATHS = 12
    display_paths = paths[:MAX_PATHS]
    if len(paths) > MAX_PATHS:
        st.caption(f"⚠️ Showing {MAX_PATHS} of {len(paths)} ancestry paths for readability.")

    dot_src = build_dot_tree(
        comp, display_paths, req_df, result_dfs, months, stock, prod_summary
    )
...
def run_mrp(bom_file, req_file, prod_file):
    ...
    # FIX: normalise Alt to integer string to prevent join failures
    ...
    # FIX: same Alt normalisation as BOM
    ...
    for m in months:
        req[m] = pd.to_numeric(
            req[m].astype(str).str.replace(",","",regex=False).str.strip(),
            errors="coerce"
        ).fillna(0)
    ...
    def make_report(gross_agg_df, comp_col):
        ...
        for comp, grp in gross_agg_df.groupby(comp_col):
            ...
            for _, row in grp.sort_values("Month_Order").iterrows():
                ...
                results.append({"Component":comp,"Description":desc,
                                 "Month":row["Month"],"Gross_Requirement":gr,
                                 "Stock_Used":consumed,"Shortage":shortage,
    ...
    for col_ui, lbl, df in zip(
        [col1, col2, col3, col4], ["L1","L2","L3","L4"],
        [result_l1, result_l2, result_l3, result_l4]
    ):
    ...
    def show_verify(tab, result_df, target, level_label):
        with tab:
            ...
            st.dataframe(
                t[["Month","Gross_Requirement","Stock_Used","Shortage","Stock_Remaining"]]
    ...
    with tab3:
        ...
        st.caption(f"Phantom rows passed through to L4: "
                   f"{len(l3[l3['L3_Ph'].apply(is_phantom)]):,}")
    ...
    final_cols = ["Component","Component descri","Procurement type","Special procurement",
                  "Confirmed_Qty","Open_Production_Qty","Stock"] + month_cols
    for c in final_cols:
    ...
    st.download_button(
        ...
        data=buf, file_name="mrp_final.xlsx",
    ...
    # ── Return everything needed for search ───────────────────────
    return dict(
...
# Show search section if results are available (persists after run)
