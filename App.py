...
st.title("📊 MRP Shortage Tool (SAP Logic)")
...
if st.button("Run MRP"):
    ...
    if bom_file and req_file:
        ...
        for lvl in range(1, max_level + 1):
            ...
            # Override phantom (skip multiplication)
            merged.loc[merged["IsPhantom"], "Required"] = merged.loc[merged["IsPhantom"], "Demand"]
        ...
        if results:
            ...
            demand = all_req.groupby(
                ["Component", "Month"]
            )["Required"].sum().reset_index()
            ...
            st.warning("No requirement generated")

    else:
        st.info("Upload both BOM and Requirement file")