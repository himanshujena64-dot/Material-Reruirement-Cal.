...
if check_password():
    ...
    if bom_file and req_file:
        if st.sidebar.button("🚀 Run MRP Engine"):
            ...
            # Handle different Qty column names
            qty_col = "Comp. Qty (CUn)" if "Comp. Qty (CUn)" in bom.columns else "Quantity"
            bom["Quantity"] = pd.to_numeric(bom[qty_col], errors="coerce").fillna(0)
            
            stock["Stock"] = pd.to_numeric(stock["Quantity"], errors="coerce").fillna(0)
            stock_dict = stock.set_index("Component")["Stock"].to_dict()

            # 3. IDENTIFY PHANTOM PARENTS
            # We map every component to its SP type to know if it is a phantom
            ...
            for i in range(len(bom)):
                ...
                # Logic to find the immediate parent
                p_id = bom.loc[i, "BOM Header"] if lvl == 1 else stack.get(lvl - 1)
                parents.append(p_id)
                
                # Logic: Check if that parent's SP is 50
                p_sp = str(phantom_map.get(p_id, "0")).strip()
                parent_is_phantom.append(True if p_sp == "50" else False)
                
                stack[lvl] = comp
            
            bom["Parent Component"] = parents
            bom["Parent_Is_Phantom"] = parent_is_phantom
            ...
            for lvl in range(1, max_level + 1):
                ...
                # --- THE CORE PHANTOM RULE ---
                # If Parent is Phantom (SP 50), the usage multiplier is 1.
                # Otherwise, use the actual Quantity from the BOM.
                ...
                # Next level explosion uses the Shortage as the new Demand
