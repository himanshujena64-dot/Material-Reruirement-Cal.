...
if check_password():
    ...
    if bom_file and req_file:
        ...
        if st.sidebar.button("🚀 Run MRP Engine"):
            ...
            status_text.text("Running MRP Explosion (Stock-Aware)...")

            # Melt Requirements to Long Format
            ...
            for lvl in range(1, max_level + 1):
                ...
                # Prepare for next level explosion: ONLY the shortage is passed down
                current = grouped[["Component", "Month", "Alt", "Shortage"]].copy()
                current = current.rename(columns={"Shortage": "Demand"})
            
            progress_bar.progress(90)

            if results:
                all_req = pd.concat(results, ignore_index=True)
                # Final aggregation for the report
                demand_final = all_req.groupby(["Component", "Month"])["Required"].sum().reset_index()
                pivot = demand_final.pivot(index="Component", columns="Month", values="Required").fillna(0).reset_index()
                ...
                st.dataframe(
                    pivot.style.applymap(
                        lambda x: 'color: red; font-weight: bold' if isinstance(x, (int, float)) and x < 0 else None
                    ), 
